...
- `dashboard.py` — the web app (static CSV mode + live UDP mode): single page with Drive/Tune/Captures/Settings tabs polling JSON endpoints (`/data`, `/settings`, `/analyze`, `/captures`, `/capture/*`, `/capture/auto*`), Chart.js CDN, stdlib `http.server`. `CaptureController` (manual start/stop/save) and `AutoLapRecorder` (opt-in, saves each completed lap automatically) run independently off the same UDP feed. `_BUFFER_LEN` (Drive tab's live rolling buffer) is separate from and much larger than a typical session, but is still a cap — see TODO.md before assuming it's unbounded. `make_live_server`'s `feed()` wraps each packet's processing in its own `except Exception` (logged, skipped) — anything added there that can throw (especially filesystem I/O like `captures.save`) must stay inside that per-packet try, not escape to the outer `except OSError`, which is reserved for the one-time UDP bind failure and previously mislabeled a `captures.save` `PermissionError` as "cannot bind" while silently killing the feed thread (see TODO.md). Start/stop capture controls exist in **two** places in `_PAGE` — Drive tab (`#drive-capture-start`/`#drive-capture-stop`/`#drive-capture-status-text`, for quick access without switching tabs) and Captures tab (`#capture-start`/`#capture-stop`/`#capture-status-text`) — both call the same `/capture/start`/`/capture/stop` endpoints and are kept in sync by one shared `refreshCaptureStatus()` (called every `poll()` cycle, not just on tab activation, so the Drive tab's status stays live while driving). Adding a third entry point: extend `refreshCaptureStatus()` again rather than writing a separate status-sync function.
